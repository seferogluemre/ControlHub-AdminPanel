...
export abstract class UsersService {
  ...
  static async store(payload: UserCreatePayload): Promise<User> {
    try {
      ...
      if (file) {
        const createdFile = await FilesService.store({
          file,
          type: FileType.USER_IMAGE,
        });
        ...
        updates.image = createdFile.path;
      }
  ...
  static async update(id: string, payload: UserUpdatePayload): Promise<User> {
    try {
      ...
      if (file !== undefined) {
        if (user.imageAsset) {
          await FilesService.destroy(user.imageAsset.uuid);
        }
        ...
        if (file !== null) {
          ...
          updates.imageAsset = { connect: { id: createdFile.id } };
          updates.image = createdFile.path;
        } else {
