...
export default function EditDepartment() {
  const { id } = useParams({ from: "/(admin)/global-settings/departments/edit/$id" });
  ...
  const [selectedAgents, setSelectedAgents] = useState(department?.members || []);
  const [unselectedAgents, setUnselectedAgents] = useState(
    agentsData.filter((agent) => !department?.members.some((member) => member.name === agent.name)),
  );
  ...
  const handleSave = () => {
    ...
    // Mevcut departmanı departmentsData'da bul ve güncelle
    const departmentIndex = departmentsData.findIndex((d) => d.id === id);

    if (departmentIndex !== -1) {
      // Güncellenmiş departman objesi oluştur
      ...
      // DepartmentsData'da güncelle
      departmentsData[departmentIndex] = updatedDepartment;
        ...
        <CardContent className="p-6 space-y-4">
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="1 - Financial İşlemler"
              required
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              rows={4}
              placeholder="Para yatırma, para çekme ve üye hesaplarıyla alakalı yapılan manuel işlemler ile alakalı yaşanan sorunlara yönelik aktif departmandır."
            />
          </div>
        </CardContent>
      </Card>
